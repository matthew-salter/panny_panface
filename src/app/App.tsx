...
import MicrophonePermissionPane from "./components/MicrophonePermissionPane";
...
function App() {
  ...
  // Permission pane state - use localStorage to ensure it persists correctly
  const [showPermissionPane, setShowPermissionPane] = useState<boolean>(true);
  
  // Force the permission pane to show on initial load
  useEffect(() => {
    // Reset permission pane visibility on component mount
    setShowPermissionPane(true);
  }, []);
  ...
  // Handler for the Continue button in the permission pane
  const handleContinue = () => {
    setShowPermissionPane(false);
    logClientEvent({ type: 'permission.microphone.accepted' }, 'permission_pane');
  };

  return (
    <div className="flex flex-col min-h-screen">
      {/* Microphone Permission Pane */}
      {showPermissionPane && <MicrophonePermissionPane onContinue={handleContinue} />}
